...
from ..parsers.tmdl_semantic import (
    canonicalize_tmdl_bytes,
    diff_models,
    parse_model,
    scope_has_semantic_model,
    split_semantic_models,
    summary_lines,
)
    ...
    for obj in objects:
        ...
        if isinstance(content, dict):
            ...
            for sub_path in sorted(content):
                ...
                if sub_path.endswith(".tmdl"):
                    # Canonicalize TMDL so the state baseline is byte-stable across
                    # pulls (TICK-041 ACT-002): kills lineageTag/annotation/ordering/
                    # CRLF churn. Fail-closed — only reorders, never drops content.
                    blob = canonicalize_tmdl_bytes(blob)
...
def diff(
    ...
    *,
    ...
    For a scope holding a Power BI ``.SemanticModel``, the signal is a CONTENT
    (semantic) diff — model objects, not raw TMDL text — so cosmetic Fabric
    serialization churn does not register as drift (TICK-041). The raw unified
    text diff stays available as a debug view via ``raw=True``, never as the
    signal. All other scopes keep the byte-level comparison.

    Args:
        config: Loaded project configuration.
        env: Target environment.
        scope: Asset scope.
        show_content: If True, print unified diffs for modified files (byte scope).
        file_filter: Optional filter for specific files.
        raw: For a semantic scope, also print the raw TMDL unified diff (debug).

    Returns:
        Byte scope — dict with keys: added, removed, modified, identical (paths).
        Semantic scope — dict with keys: semantic (True), content_drift (bool),
        totals (added/removed/changed counts), models (name -> SemanticDiff).
    """
    overlay = load_overlay(config.root, env)
    src_dir = config.src_dir(scope)
    patches_dir = config.patches_dir(env) / scope
    ...
    # Semantic scope (Power BI .SemanticModel): compare CONTENT, not raw TMDL,
    # so cosmetic serialization churn (lineageTag/annotation/ordering/auto-date/
    # CRLF) cannot cry wolf as a "wide drift" (TICK-041).
    if scope_has_semantic_model(local_files) or scope_has_semantic_model(state_files):
        return _diff_semantic(local_files, state_files, raw=raw)
...
def _diff_semantic(
    ...
    *,
    ...
    """Content-aware diff for scopes holding a ``.SemanticModel`` (TICK-041).

    Compares model objects (tables / columns / measures / partitions /
    relationships) between the assembled ``src`` (proposed) and the pulled
    ``state`` (baseline/live). The verdict ``content_drift`` is the gate signal:
    ``False`` when ``src ⊇ live`` with no regression (the operator only added
    objects, or nothing changed but serialization) — the case that used to
    show as a spurious wide drift.
    """
    local_models = split_semantic_models(local_files)
    state_models = split_semantic_models(state_files)

    models: dict[str, object] = {}
    totals = {"added": 0, "removed": 0, "changed": 0}
    content_drift = False

    for name in sorted(set(local_models) | set(state_models)):
        baseline = parse_model(state_models.get(name, {}), name=name)
        proposed = parse_model(local_models.get(name, {}), name=name)
        sd = diff_models(baseline, proposed)
        models[name] = sd
        for k, v in sd.counts().items():
            totals[k] += v
        content_drift = content_drift or sd.has_content_loss

        print(f"\nSemanticModel: {name}")
        for line in summary_lines(sd):
            print(f"  {line}")

    print()
    if content_drift:
        print(
            f"RESULT: CONTENT DRIFT — {totals['removed']} removed, "
            f"{totals['changed']} changed, {totals['added']} added "
            f"(removed/changed would alter live content — review before push)"
        )
    else:
        print(
            f"RESULT: CONTENT CLEAN — assembled src ⊇ live "
            f"(+{totals['added']} new object(s), 0 removed, 0 changed; "
            f"any text difference is serialization noise)"
        )

    # The raw TMDL unified diff is a debug view, never the signal. Only the
    # byte-modified files, and only when explicitly requested.
    if raw:
        modified = sorted(
            p for p in set(local_files) & set(state_files)
            if local_files[p] != state_files[p]
        )
        if modified:
            print(f"\n[raw] {len(modified)} file(s) differ textually (serialization):")
            for path in modified:
                print(f"  ~ {path}")
                _print_unified_diff(state_files[path], local_files[path], path)
    elif totals == {"added": 0, "removed": 0, "changed": 0}:
        print("  (text differs only in serialization — see --raw for the byte diff)")
