...
Manual setup (no Claude Code) is the same: copy the `core/templates/scenario-databricks-to-powerbi/` tree to `distributions/reference/projects/<project>/` and edit the placeholders. Documentation in [`core/templates/README.md`](../../core/templates/README.md).
...
You should see green ticks for Python, dependencies, project config, credentials, and Databricks reachability. If anything is red, the message explains what to set.
...
- **`MissingSchemaError` on PBIR pull**: the workspace was bootstrapped via legacy Power BI Desktop "Publish" before PBIR. See [`core/docs/fabric_404_vs_403.md`](../../core/docs/fabric_404_vs_403.md) for the diagnostic flow.
- **Notebook converter tagging output as `heavy` or `impossible`**: the converter flagged constructs it can't safely auto-rewrite. Inspect the converter output — these are your call. Tagging is documented in [`core/parsers/databricks_migration_assess.py`](../../core/parsers/databricks_migration_assess.py).
...
- Read [`core/conventions/sanitization-patterns.md`](../../core/conventions/sanitization-patterns.md) before contributing back — sample data and examples must use generic placeholders.
