...
FABRIC_SCOPE = [f"{FABRIC_RESOURCE}/.default"]
...
class FabricAuthenticator:
    ...
    def get_token(self, *, force_refresh: bool = False) -> str:
        """Return a valid bearer token, fetching one if needed.

        Tokens are cached in-process. Pass ``force_refresh=True`` after a 401
        to bypass the cache.
        """
        if self._cached_token and not force_refresh:
            return self._cached_token
        if self.method == "service_principal":
            token = self._get_token_sp()
        elif self.method == "az_cli":
            token = self._get_token_az_cli()
