...
- `core/conventions/powerbi-workflow-choice.md` — engine path vs MCP `serena` path decision tree.
...
- **`.mcp.example.json`** published as the canonical setup template for fresh clones, including the canonical names of the wrapped MCP servers (`dde` → `databricks-mcp-server` from the Databricks AI Dev Kit; `serena` → Microsoft `analysis-services.powerbi-modeling-mcp`).
