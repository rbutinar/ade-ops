...
**Scenario C — demo inherits client creds on a shared machine** (safety, observed 2026-06-02):

You run a demo / public-reference project whose `credentials.yaml` and
`project.yaml` use `${DATABRICKS_HOST}`/`${DATABRICKS_TOKEN}` on a machine
that **also** holds a client's Databricks creds in user-scope env vars (set
for daily client work). With no demo-specific values, the demo silently
resolves to the **client production workspace** — `preflight` reports
"reachable ✓" while pointing at the wrong identity, and the naive fix
(align `DEMO_USER_EMAIL` to the token) makes the destination path valid and
proceeds to write into the client workspace.

`preflight` now surfaces the resolved **token identity + host/token
provenance** and flags an identity-vs-target mismatch as a hard failure
(TICK-008) — but the durable fix is isolation. **Recommended default for any
machine that also does client work: give the demo project-scoped literals,
not env-var references.**

```yaml
# <demo-project>/config/credentials.yaml  (gitignored)
databricks:
  host: https://dbc-XXXXXXXX-XXXX.cloud.databricks.com   # the DEMO / Free Edition workspace (literal)
  token: dapiXXXXXXXXXXXXXXXXXXXXXXXXXXXX                 # a token scoped to that demo workspace
```

And set `platforms.databricks.host` in `project.yaml` to the literal demo
host (not `${DATABRICKS_HOST}`), OR override it per seat in
`.claude/settings.local.json`. Principle: **a demo on a client machine must
declare its target explicitly — never let it fall through to ambient client
env vars.** Run `python -m core.cli preflight --env <env>` and confirm the
surfaced identity is the demo account before any push.
...
- **Hardcoding the workspace host in `project.yaml`** — `platforms.databricks.host` is committed. A literal value (e.g. `host: https://adb-xxxxxxxx.azuredatabricks.net`) forces every seat on the same project to target the same workspace, defeats sanitization (real hosts leak to publishes), and breaks the standard multi-seat workflow. Use `host: ${DATABRICKS_HOST}` and let each seat resolve it via the modality 2 or 3 path above. The `reference` distribution's `databricks-fabric-migration` project is the gold standard. Surfaced 2026-05-28 — a sanitized placeholder host had remained committed on a client project, causing the seat to talk to a non-existent workspace until the env-var convention was applied.
