...
- [`lab-to-distribution-port.md`](lab-to-distribution-port.md) — the discipline
  for porting lab `core/` into a disjoint-git-world distribution (a client's
  DevOps repo): run the divergence-report, reconcile companions together, validate
  with a real op, never port the security set. Pairs with `tools/core_divergence.py`.
