...
- Plus the "fewer pieces" **Databricks → Power BI (Import mode, no Fabric)**
  variant — Power BI Pro is enough, no capacity / lakehouse / DirectLake
...
| Project | Path | Status |
|---|---|---|
| Playground (Databricks-only, ~5 min) | `projects/playground/` | reference |
| Databricks → Fabric migration | `projects/databricks-fabric-migration/` | reference |
| Databricks → Power BI (Import) | `projects/acme-powerbi/` | reference |

`playground/` is the fastest start (synthetic data, no Fabric/PBI) — the
`databricks-only` onboarding scenario points here.
