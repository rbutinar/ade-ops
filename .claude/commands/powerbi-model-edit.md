...
You are editing a **running** Power BI Desktop model via the `serena` MCP server. Operations are interactive: measures, tables, DAX validation, TMDL export.
...
- `serena` MCP server configured in `.mcp.json` (see ONBOARDING.md Step 7)
...
```
mcp__serena__connection_operations(operation="ListLocalInstances")
```
...
```
mcp__serena__connection_operations(operation="Connect", dataSource="localhost:{port}")
```
...
```
mcp__serena__measure_operations(operation="List")
```
...
```
mcp__serena__dax_query_operations(
  operation="Validate",
  query="EVALUATE {{ {dax} }}"
)
```
...
```
mcp__serena__measure_operations(
  operation="Create",
  definitions=[{
    tableName: "{table}",
    name: "{name}",
...
```
mcp__serena__table_operations(operation="List")
```

#### `validate "{dax}"`

```
mcp__serena__dax_query_operations(operation="Validate", query="EVALUATE {dax}")
```
...
```
mcp__serena__database_operations(
