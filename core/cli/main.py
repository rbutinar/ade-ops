...
@click.option("--raw", is_flag=True,
              help="For a semantic model, also show the raw TMDL text diff (debug)")
def diff(project, env, scope, no_content, filter_pattern, raw):
    """Compare assembled local against pulled remote state."""
    config = _load(project)
    try:
        op_diff(config, env, scope, show_content=not no_content,
                file_filter=filter_pattern, raw=raw)
...
def pbir_create(name, project, env, spec, model_id, workspace_display_name,
    ...
    is documented in ``.claude/skills/pbir-create/SKILL.md``.
...
@click.option("--slim", is_flag=True, default=False,
              help="Drop slide layouts the deck does not use, so output from a "
                   "rich starter pack is not bloated by unused layouts + media "
                   "(overrides meta.slim_layouts).")
def deck_build(spec, output, render, slim):
    ...
    # --slim overrides meta.slim_layouts; unset defers to the spec (None).
    try:
        report = build_from_spec(
            spec_path, output_path=output_path,
            slim_layouts=True if slim else None,
        )
    except (BuildError, DecksConfigError) as exc:
        click.echo()
        click.echo(click.style(f"ERROR: {exc}", fg="red", bold=True))
    ...
    if report.layouts_pruned:
        click.echo(f"  slimmed  : {report.layouts_pruned} unused layout(s) pruned")
