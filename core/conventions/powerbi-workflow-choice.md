...
> **MCP path** (Microsoft Power BI Modeling MCP via `serena`, requires
...
| Aspect | Engine path | MCP path (`serena`) |
|---|---|---|
...
| External dependency | None (Python `core/parsers/` + `core/platforms/powerbi/`) | Power BI Desktop running + `.pbip` file open + `serena` MCP server registered |
| Setup cost | Zero — comes with ade-ops install | Install VSIX of `analysis-services.powerbi-modeling-mcp` (see `.mcp.example.json` `_setup_serena_prereq`) + binding to local Desktop process |
...
`/powerbi-model-edit` (which talks to `serena` MCP) is faster and more
...
1. Install Node.js LTS (`winget install OpenJS.NodeJS.LTS`) — required
   ...
   strictly required for `serena` alone but expected by the bundle.
...
3. Configure `.mcp.json` (copy from `.mcp.example.json` template,
   replace `<your-user>` placeholders). The `serena` block is already
   shaped for you.
...
5. Restart Claude Code so the new `.mcp.json` is picked up. `claude
   mcp list` should now show `serena` in addition to `dde` and
   `playwright`.
...
- [`.mcp.example.json`](../../.mcp.example.json) — `serena` server registration template + setup prereqs
