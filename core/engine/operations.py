    ...
    *,
    ...
    for obj in objects:
        ...
        if isinstance(content, dict):
            # Folder payload (e.g. PBIP / TMDL parts). Wipe any previous
            # contents under this item so deletes round-trip cleanly.
            ...
            if hasattr(connector, "materialize_siblings") and callable(
                ...
                for sibling_path, sibling_blob in siblings.items():
                    ...
                    target.parent.mkdir(parents=True, exist_ok=True)
                    target.write_bytes(sibling_blob)
        else:
...
def push(
    ...
    *,
    ...
    # Assemble files
    files = assemble_scope(src_dir, overlay, patches_dir)
    ...
    if file_filter:
        ...
        print(f"[FILTER] {len(files)} files matching '{file_filter}'")
...
def diff(
    ...
    *,
    ...
    # Assemble local
    local_files = assemble_scope(src_dir, overlay, patches_dir)
